...
class VisualpingConnector(BaseConnector):
    ...
    def _make_rest_call(self, endpoint, action_result, method="get", **kwargs):
        ...
        try:
            ...
            error_message = (
                "Error connecting to server. No connection adapters were found for %s"
                % (url)
            )
            return RetVal(
    ...
    def _handle_test_connectivity(self, param):
        ...
        self.save_progress(
            "Connecting to {}{} endpoint".format(self._base_url, HEALTH_ENDPOINT)
        )
        # make rest call
        ret_val, response = self._make_rest_call(
            HEALTH_ENDPOINT, action_result, params=None, headers=None
        )
    ...
    def _handle_list_jobs(self, param):
            ...
            if isinstance(jobs, dict):
                if "active" in jobs:
                    ...
                    return action_result.set_status(
                        phantom.APP_SUCCESS, "No active jobs found"
                    )
            else:
                return action_result.set_status(
                    phantom.APP_ERROR, UNEXPECTED_RESPONSE_MSG
                )
        except Exception as e:
            error_message = self._get_error_message_from_exception(e)
        ...
        return action_result.set_status(
            phantom.APP_SUCCESS, "Successfully fetched the job(s)"
        )
    ...
    def _handle_show_result(self, param):
        ...
        ret_val, response = self._make_rest_call(
            "/api/job/lastresult/{0}".format(job_id),
            action_result,
            params=None,
            headers=None,
        )
        ...
        if not response:
            return action_result.set_status(
                phantom.APP_ERROR, "Couldn't find data for the specified ID"
            )
        ...
        return action_result.set_status(
            phantom.APP_SUCCESS, "Successfully fetched the result"
        )
    ...
    def _handle_get_images(self, param):
        ...
        return action_result.set_status(
            phantom.APP_SUCCESS, "Successfully fetched the images"
        )
